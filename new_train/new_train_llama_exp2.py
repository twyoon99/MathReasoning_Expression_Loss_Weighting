...
def mark_math_tokens(text, tokenizer, input_ids_sample):
    ...
    for tok in decoded_tokens:
        ...
        start = pointer if start == -1 else start
        end = start + len(tok)
...
class WeightedLossTrainer(Trainer):
    def __init__(self, *args, math_weight=2.0, weight_decay=0.01, **kwargs):
    ...
    def create_optimizer(self):
        if self.optimizer is None:
            grouped_params = [
                {"params": [p for n, p in self.model.named_parameters() if "bias" not in n and "LayerNorm.weight" not in n], "weight_decay": self.weight_decay},
                {"params": [p for n, p in self.model.named_parameters() if "bias" in n or "LayerNorm.weight" in n], "weight_decay": 0.0},
            ]
...
base_model = AutoModelForCausalLM.from_pretrained(model_name, quantization_config=bnb, device_map="auto")
...
training_args = TrainingArguments(
    output_dir="/home/woong/MediTOD_DST/math-split-slm/new_outputs/checkpoints/new_model_llama_exp2",
...
trainer.train()
model.save_pretrained("/home/woong/MediTOD_DST/math-split-slm/new_outputs/checkpoints/new_model_llama_exp2")